...
import pocketmine.nbt.tag.CompoundTag;
import pocketmine.plugin.Plugin;

import java.util.List;
import java.util.UUID;

public class OfflinePlayer implements IPlayer, Metadatable
{
  ...
  public OfflinePlayer (Server server, UUID uuid, String name)
  {
	...
	nbt = this.server.getOfflinePlayerData (name, false);
      ...
      {
	throw new
	  IllegalArgumentException ("Name and UUID cannot both be null");
      }
  ...
  @Override public Long getFirstPlayed ()
  {
    return this.namedTag !=
      null ? this.namedTag.getLong ("firstPlayed") : null;
  }

  @Override public Long getLastPlayed ()
  {
    return this.namedTag !=
      null ? this.namedTag.getLong ("lastPlayed") : null;
  }
  ...
  {
    this.server.getPlayerMetadata ().setMetadata (this, metadataKey,
						  newMetadataValue);
  }
  ...
  {
    this.server.getPlayerMetadata ().removeMetadata (this, metadataKey,
						     owningPlugin);
  }
}
